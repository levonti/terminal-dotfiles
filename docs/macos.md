...
Затем клонируйте репозиторий и установите ссылки, включая Bash, по [README](../README.md#установка-с-чистого-устройства). Если текущий shell не `/bin/zsh`, выполните `chsh -s /bin/zsh` и войдите в macOS заново. Откройте Kitty из Applications, проверьте prompt, `z` и `zi`. Запустите `bash` и убедитесь, что метка сменилась на `bash`, а `z` и `zi` доступны; затем выполните `exit`.
