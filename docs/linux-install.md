...
Клонируйте репозиторий и установите ссылки, включая Bash, по [README](../README.md#установка-с-чистого-устройства). Установщик сохранит существующий `~/.bashrc` как `~/.bashrc.local`; проверьте, что в нём нет повторной инициализации Starship и zoxide. Затем смените shell по умолчанию и войдите в графическую сессию заново:

```sh
chsh -s "$(command -v zsh)"
```

После нового входа откройте Kitty из меню приложений, выполните `echo "$SHELL"`, `type z`, `type zi` и проверьте prompt. Запустите `bash`: метка должна смениться на `bash`, а `z` и `zi` должны остаться доступными. Выйдите из Bash командой `exit`. Если на устройстве нет `sudo`, выполняйте привилегированные команды от root по правилам этой системы.
