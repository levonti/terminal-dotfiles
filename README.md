...
Общая конфигурация Kitty, zsh, дополнительного Bash, Starship и zoxide для macOS, Ubuntu 24.04 GNOME, Ubuntu 24.04 KDE Plasma и Debian 13 KDE Plasma. Цель — одинаковые prompt, цвета, шрифт и сочетания клавиш внутри терминала на разных устройствах.
...
| `bash` (дополнительно) | `~/.bashrc` | Starship для интерактивного Bash и необязательный `~/.bashrc.local` |
...
Если вы запускаете `bash` из zsh и хотите видеть в нём тот же prompt с меткой `bash`, подключите отдельный пакет:

```sh
stow --no-folding --simulate --verbose --target="$HOME" bash
stow --no-folding --target="$HOME" bash
```

Если `~/.bashrc` уже существует, сначала сохраните его настройки и разрешите конфликт перед Stow. Пакет не входит в базовую установку, поскольку на Linux этот файл обычно уже содержит пользовательские настройки. Интерактивный Bash без входа в систему читает `~/.bashrc`; для Bash с `--login` существующий `~/.bash_profile` должен также подключать `~/.bashrc` при необходимости. [Правила запуска Bash](https://www.gnu.org/software/bash/manual/html_node/Bash-Startup-Files.html).
...
Общий `.zshrc` сначала добавляет стандартные каталоги пользовательских бинарников и Homebrew в `PATH`, затем читает `~/.zshrc.local` перед инициализацией инструментов и `~/.zshrc.local.post` после основных настроек. Дополнительный `.bashrc` читает `~/.bashrc.local` перед Starship. Kitty в конце читает необязательный `~/.config/kitty/kitty.local.conf`. Для Starship возможна полная локальная замена через `STARSHIP_CONFIG`. Подробности: [локальные переопределения](docs/local-overrides.md).

## SSH из Kitty

Интерактивная команда `ssh` из zsh в Kitty использует совместимый `TERM=xterm-256color` для неизвестных хостов. Полный режим `kitten ssh` включается только для хостов, совпавших с приватными масками. Дополнительный Bash-пакет настраивает prompt, но не перенимает маршрутизацию SSH из zsh. Список масок по умолчанию пуст. Правила, границы изменений на сервере и проверка описаны в [руководстве по SSH](docs/ssh.md).
